...
@register_model
def edgevit_xxs(pretrained=True, resolution=320, **kwargs):
    model = EdgeVit(
        depth=[1, 1, 3, 2],
        embed_dim=[32, 48, 96, 176], head_dim=20, mlp_ratio=[4] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1], img_size=resolution, **kwargs)
    model.default_cfg = _cfg()
    return model


@register_model
def edgevit_xs(pretrained=True, resolution=320, **kwargs):
    model = EdgeVit(
        depth=[1, 1, 3, 1],
        embed_dim=[32, 48, 120, 224], head_dim=32, mlp_ratio=[4] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1], img_size=resolution, **kwargs)
    model.default_cfg = _cfg()
    return model
...
@register_model
def edgevit_s(pretrained=True, resolution=320, **kwargs):
    model = EdgeVit(
        depth=[1, 2, 5, 3],
        embed_dim=[32, 64, 144, 288], head_dim=32, mlp_ratio=[4] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1], img_size=resolution, **kwargs)
    model.default_cfg = _cfg()
    return model
